...
fn is_wangp_download(file_name: &str) -> bool {
    const EXT_OK: &[&str] = &["png", "jpg", "jpeg", "webp", "gif", "bmp", "tif", "tiff", "jfif",
        "mp4", "mov", "avi", "mkv", "webm", "m4v", "mpeg", "mpg", "ogv",
        "wav", "mp3", "ogg", "flac", "zip", "json", "lset", "srt", "vtt", "txt"];
    const SKIP: &[&str] = &["tmp", "temp", "crdownload", "part", "partial", "download", "opdownload", "lock", "bak", "lnk"];
    if file_name.starts_with('.') || file_name.starts_with("~$") { return false; }
    let ext = Path::new(file_name).extension().and_then(|x| x.to_str()).unwrap_or("").to_lowercase();
    if SKIP.contains(&ext.as_str()) || !EXT_OK.contains(&ext.as_str()) { return false; }
    if ext == "zip" || ext == "json" || ext == "lset" { return true; }
...
fn has_wangp_stamp(s: &str) -> bool {
    let b = s.as_bytes();
    if b.len() < 22 { return false; }
    for i in 0..=(b.len() - 22) {
        if b[i] == b'-'
            && b[i+1..i+5].iter().all(|c| c.is_ascii_digit()) && b[i+5] == b'-'
            && b[i+6..i+8].iter().all(|c| c.is_ascii_digit()) && b[i+8] == b'-'
            && b[i+9..i+11].iter().all(|c| c.is_ascii_digit()) && b[i+11] == b'-'
            && b[i+12..i+14].iter().all(|c| c.is_ascii_digit()) && b[i+14] == b'h'
            && b[i+15..i+17].iter().all(|c| c.is_ascii_digit()) && b[i+17] == b'm'
            && b[i+18..i+20].iter().all(|c| c.is_ascii_digit()) && b[i+20] == b's'
        {
            return true;
        }
    }
...
fn has_wangp_seed(s: &str) -> bool {
    ...
    while let Some(i) = rest.find("_seed") {
        if rest.as_bytes().get(i + 5).is_some_and(|c| c.is_ascii_digit()) {
            return true;
        }
        rest = &rest[i + 5..];
    }
...
#[tauri::command] pub fn downloads_since(since_ms: i64) -> serde_json::Value {
    ...
    if let Ok(rd) = std::fs::read_dir(home_dir().join("Downloads")) {
        for e in rd.flatten() {
            ...
            if !is_wangp_download(&name) { continue; }
    ...
    if let Ok(rd) = std::fs::read_dir(staging_dir()) {
        for e in rd.flatten() {
            let stale = e.metadata().ok().and_then(|m| m.modified().ok())
                ...
                .map(|d| d.as_secs() > 7 * 86400).unwrap_or(false);
            if stale { let _ = std::fs::remove_file(e.path()); }
        }
    }
}
...
fn unique_in(dir: &Path, fname: &str) -> PathBuf {
    ...
    dir.join(format!("{stem}-{}.{ext}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_millis()).unwrap_or(0)))
}
/// Move src → dst (atomic rename, copy+verify+remove across drives).
fn move_file_verified(src: &Path, dst: &Path) -> Result<(), String> {
    if std::fs::rename(src, dst).is_err() {
        std::fs::copy(src, dst).map_err(|e| e.to_string())?;
        let (a, b) = (std::fs::metadata(src).map(|m| m.len()).unwrap_or(0),
            std::fs::metadata(dst).map(|m| m.len()).unwrap_or(1));
        if a != b { let _ = std::fs::remove_file(dst); return Err("copy verification failed (size mismatch) — staged copy kept".into()); }
        std::fs::remove_file(src).map_err(|e| e.to_string())?;
    }
    Ok(())
}
fn embed_mode(opts: Option<&serde_json::Value>) -> String {
    ...
    if let Some(m) = opts.as_ref().and_then(|o| o.get("mode")).and_then(|v| v.as_str()) {
        if m == "native" || m == "iframe" { return m.to_string(); }
    }
    crate::base::load_config_value().get("embedMode").and_then(|v| v.as_str()).unwrap_or("native").to_string()
}

#[tauri::command]
pub async fn create_browser_view(app: tauri::AppHandle, url: Option<String>, opts: Option<serde_json::Value>) -> Result<serde_json::Value, String> {
    ...
    let (x, y, w, h) = opts.as_ref().and_then(|o| o.get("rect")).map(|r| (
    ...
    if let Some(v) = app.get_webview(GRADIO_VIEW_LABEL) { let _ = v.close(); }
    sweep_staging();
    ...
    let view = window.add_child(builder,
        tauri::LogicalPosition::new(x, y), tauri::LogicalSize::new(w, h)).map_err(|e| e.to_string())?;
    let _ = view.show();
...
#[tauri::command] pub fn destroy_browser_view(app: tauri::AppHandle) -> serde_json::Value {
    use tauri::Manager;
    if let Some(v) = app.get_webview(GRADIO_VIEW_LABEL) { let _ = v.close(); }
...
#[tauri::command] pub fn hide_browser_view(app: tauri::AppHandle) -> serde_json::Value {
    use tauri::Manager;
    if let Some(v) = app.get_webview(GRADIO_VIEW_LABEL) { let _ = v.hide(); }
    serde_json::json!({"ok": true})
}
#[tauri::command] pub fn detach_browser_view(app: tauri::AppHandle) -> serde_json::Value {
    use tauri::Manager;
    if let Some(v) = app.get_webview(GRADIO_VIEW_LABEL) { let _ = v.hide(); }
    serde_json::json!({"ok": true})
}
#[tauri::command] pub fn reattach_browser_view(app: tauri::AppHandle) -> serde_json::Value {
    use tauri::Manager;
    if let Some(v) = app.get_webview(GRADIO_VIEW_LABEL) { let _ = v.show(); }
    serde_json::json!({"ok": true})
}
...
pub async fn create_term_view(app: tauri::AppHandle) -> Result<serde_json::Value, String> {
    ...
    let w = tauri::WebviewWindowBuilder::new(&app, TERM_LABEL, tauri::WebviewUrl::App("term.html".into()))
...
#[tauri::command] pub fn destroy_term_view(app: tauri::AppHandle) -> serde_json::Value {
    use tauri::Manager;
    // No-op when no term window exists (iframe flows never open one).
    if let Some(w) = app.get_window(TERM_LABEL) { let _ = w.close(); }
    serde_json::json!({"ok": true})
}
...
#[tauri::command] pub fn term_set_dock(app: tauri::AppHandle, dock: String) -> serde_json::Value {
    use tauri::{Emitter, Manager};
...
#[tauri::command] pub fn export_logs(app: tauri::AppHandle, text: Option<String>) -> Result<serde_json::Value, String> {
    use tauri_plugin_dialog::DialogExt;
    let t = text.unwrap_or_default();
    if t.trim().is_empty() { return Err("nothing to export".into()); }
    let dst = match app.dialog().file().set_file_name("wan2gp-console.log").blocking_save_file() {
        Some(p) => p.into_path().map_err(|e| e.to_string())?,
        None => return Ok(serde_json::json!({"ok": true, "cancelled": true})),
    };
...
#[tauri::command] pub fn bv_navigate(app: tauri::AppHandle, action: String) -> serde_json::Value {
    use tauri::Manager;
    ...
    if action == "reload" {
        if let Some(v) = app.get_webview(GRADIO_VIEW_LABEL) { let _ = v.reload(); }
    }
    serde_json::json!({"ok": true})
}
#[tauri::command] pub fn bv_set_zoom(app: tauri::AppHandle, factor: f64) -> serde_json::Value {
    use tauri::Manager;
    // Compositor zoom (IsZoomControlEnabled) — replaces the iframe's CSS-zoom hack.
    if let Some(v) = app.get_webview(GRADIO_VIEW_LABEL) { let _ = v.set_zoom(factor.clamp(0.25, 2.0)); }
    serde_json::json!({"ok": true})
}
...
#[tauri::command] pub fn bv_sync_bounds(app: tauri::AppHandle, x: f64, y: f64, w: f64, h: f64) -> serde_json::Value {
    use tauri::Manager;
...
#[tauri::command] pub fn bv_set_dock(app: tauri::AppHandle, dock: String) -> serde_json::Value {
    use tauri::Manager;
    let _ = dock;
    if let Some(v) = app.get_webview(GRADIO_VIEW_LABEL) { let _ = v.show(); }
    serde_json::json!({"ok": true})
...
#[tauri::command]
pub fn save_staged_download(app: tauri::AppHandle, path: String, dir: Option<String>) -> Result<serde_json::Value, String> {
    use tauri_plugin_dialog::DialogExt;
    let canon_stage = staging_dir().canonicalize().unwrap_or_else(|_| staging_dir());
    let canon_src = PathBuf::from(&path).canonicalize()
    ...
    if !canon_src.is_file() { return Err("download is no longer available (moved or deleted?)".into()); }
    let fname = canon_src.file_name().map(|n| n.to_string_lossy().to_string())
...
#[tauri::command] pub fn mirror_console(app: tauri::AppHandle, text: String) -> serde_json::Value {
    use tauri::Emitter;
...
#[tauri::command] pub fn webview_memory() -> serde_json::Value {
    ...
    for (pid, p) in sys.processes() {
        if pid.as_u32() == self_pid { self_mb = p.memory() / 1048576; }
        if p.name().to_string_lossy().to_lowercase().contains("msedgewebview2") {
            let mb = p.memory() / 1048576;
            wv_mb += mb; wv_n += 1;
            rows.push(serde_json::json!({"pid": pid.as_u32(), "mb": mb}));
        }
    }
    rows.sort_by(|a, b| b.get("mb").and_then(|x| x.as_u64()).unwrap_or(0).cmp(&a.get("mb").and_then(|x| x.as_u64()).unwrap_or(0)));
    rows.truncate(8);
