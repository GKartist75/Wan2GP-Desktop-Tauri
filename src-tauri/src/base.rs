...
pub(crate) static LAST_STATUS: OnceLock<
    Mutex<
        Option<(
            std::time::Instant,
    ...
    >,
> = OnceLock::new(); // status cache: 5s floor + site-packages mtime — pip installs invalidate instantly, dashboard paints from cache
...
pub(crate) static METRICS_CACHE: OnceLock<Mutex<Option<(std::time::Instant, serde_json::Value)>>> =
    OnceLock::new();
...
pub(crate) fn invalidate_path_cache() {
    if let Some(m) = CACHED_DATA_DIR.get() {
    ...
    if let Some(m) = CACHED_REPO_DIR.get() {
        let _ = m.lock().map(|mut g| {
            g.1 = std::time::Instant::now()
                .checked_sub(std::time::Duration::from_hours(1))
                .unwrap()
...
pub(crate) fn mutating_try(name: &str) -> Result<(), String> {
    ...
    if let Some(cur) = g.as_ref() {
        return Err(format!(
            "Another operation already running ({cur}). Wait for it to finish."
        ));
    }
    *g = Some(name.to_string());
    Ok(())
}
pub(crate) fn mutating_done() {
    if let Some(m) = MUTATING.get() {
        *m.lock().unwrap() = None;
    }
}
...
pub(crate) fn data_dir_override_file() -> PathBuf {
    home_dir().join(".wan2gp-tauri-data-dir")
}
#[allow(dead_code)]
pub(crate) fn data_dir_override_file_electron() -> PathBuf {
            ...
            if let Ok(s) = std::fs::read_to_string(&pth) {
                ...
                if !d.is_empty() {
                    ...
                    if !p.exists() {
                        ...
                        if legacy.exists() || nested.exists() {
...
pub(crate) fn default_data_dir() -> PathBuf {
    ...
    #[cfg(windows)]
    {
        for d in [
            ...
            let p = PathBuf::from(d);
            if p.join("wgp.py").exists() || p.join("Wan2GP").join("wgp.py").exists() {
                return p;
            }
        }
        let legacy = appdata_dir().join("wan2gp-desktop").join("Wan2GP");
        if legacy.join("wgp.py").exists() || legacy.join("Wan2GP").join("wgp.py").exists() {
            return legacy;
        }
        // Prefer root of current drive if writable
        let root = PathBuf::from(std::env::var("SystemDrive").unwrap_or("C:".into()) + "\\Wan2GP");
        if dir_is_writable(&root) || dir_is_writable(root.parent().unwrap_or(Path::new("C:\\"))) {
            return root;
        }
...
pub(crate) fn get_envs_file() -> PathBuf {
    get_repo_dir().join("envs.json")
}
        ...
        if wp.exists() {
            if let Ok(s) = std::fs::read_to_string(&wp) {
                if let Ok(mut v) = serde_json::from_str::<serde_json::Value>(&s) {
                    ...
                    if d {
                        let _ = atomic_write(&wp, &serde_json::to_string_pretty(&v).unwrap_or(s));
                    }
                }
            }
        }
    ...
    if f.exists() {
        if let Ok(s) = std::fs::read_to_string(&f) {
            if let Ok(mut v) = serde_json::from_str::<serde_json::Value>(&s) {
                ...
                if dirty {
                    let _ = atomic_write(&f, &serde_json::to_string_pretty(&v).unwrap_or(s));
                }
                return v;
            }
        }
...
pub(crate) fn pip_spec_ok(spec: &str) -> Result<(), String> {
    ...
    if let Some(rest) = spec.strip_prefix("https://") {
        ...
        if base.is_empty() || !ok_ext {
            return Err("bad wheel URL (https + .whl/.tar.gz/.zip only)".into());
        }
        return Ok(());
    }
    if spec.contains('/') || spec.contains('\\') {
        return Err("path separators not allowed".into());
    }
    ...
    if spec.starts_with('-') {
        return Err("pip options are not accepted here".into());
    }
    Ok(())
}
