...
pub(crate) fn pinned_python_wanted() -> String {
    ...
    if let Ok(s) = std::fs::read_to_string(repo.join("setup_config.json")) {
        if let Ok(v) = serde_json::from_str::<serde_json::Value>(&s) {
            ...
            if let Some(p) = v.get("components").and_then(|c| c.get("python")) {
                cands.push(p);
            }
            for c in cands {
                ...
                if ver.chars().filter(|c| *c == '.').count() >= 2 {
                    return ver;
                }
                ...
                if ver.starts_with("3.") {
                    if let Some(exact) = v
                        .get("components")
                        .and_then(|c| c.get("python"))
                        .and_then(|p| p.get(ver.as_str()))
                        .and_then(|e| e.get("ver"))
                        .and_then(|x| x.as_str())
                    {
                        let full = scan_version(exact);
                        if full.chars().filter(|c| *c == '.').count() >= 2 {
                            return full;
                        }
                    }
...
fn owned_uv_in(data_dir: &std::path::Path) -> Option<String> {
    ...
    if !data_dir.join(".tools").join(UV_MARKER).is_file() {
        return None;
    }
    silent_command(&p)
        .arg("--version")
...
fn owned_uv() -> Option<String> {
    owned_uv_in(&get_data_dir())
}
...
fn installer_owned_uv_in(data_dir: &std::path::Path) -> Option<String> {
    ...
    let ver = silent_command(dir.join(UV_EXE))
        .arg("--version")
        .output()
        .ok()
        .filter(|o| o.status.success())
    ...
    crate::base::push_log(
        &format!("[*] uv installed into launcher tools ({ver})."),
        "setup",
    );
...
fn copy_owned_uv_in(data_dir: &std::path::Path) -> Option<String> {
    if owned_uv_in(data_dir).is_some() {
        return owned_uv_in(data_dir);
    }
    let src = path_uv_exe()?;
    ...
    if let Some(parent) = src.parent() {
        #[cfg(windows)]
        let siblings = ["uv.exe", "uvx.exe", "uvw.exe"];
        #[cfg(not(windows))]
        let siblings = ["uv", "uvx"];
        for name in siblings {
            let s = parent.join(name);
            if s.is_file() {
                let _ = std::fs::copy(&s, dir.join(name));
            }
        }
    }
...
fn ensure_owned_uv_in(data_dir: &std::path::Path) -> Option<String> {
    if let Some(owned) = owned_uv_in(data_dir) {
        return Some(owned);
    }
    if let Some(fresh) = installer_owned_uv_in(data_dir) {
        return Some(fresh);
    }
    copy_owned_uv_in(data_dir)
}
fn ensure_owned_uv() -> Option<String> {
    ensure_owned_uv_in(&get_data_dir())
}
...
fn uv_command() -> String {
    if let Some(owned) = owned_uv() {
        return owned;
    }
    if let Some(snap) = ensure_owned_uv() {
        return snap;
    }
    "uv".into()
}
...
pub(crate) fn tool_found(tool: &str) -> bool {
    if known_tool_path(tool).is_some() {
        return true;
    }
    if tool == "uv" && owned_uv().is_some() {
        return true;
    }
    #[cfg(windows)]
    {
...
async fn ensure_uv_python(
    ...
    emit(&format!(
        "[*] Ensuring Python {wanted} via {uv_bin} (setup.py needs this exact version)…\n"
    ));
    ...
    let find = || {
        silent_command(uv_bin.as_str())
            .args(["python", "find", wanted])
            .output()
            .ok()
            .and_then(|o| {
                if o.status.success() {
    ...
    if let Some(p) = find() {
        ...
        emit(&format!(
            "[!] Found Python at {p} but it won't run — forcing a clean reinstall…\n"
        ));
    }
    // 2) Provision via uv.
    ...
    if !dl_ok {
        emit(&format!(
            "[!] uv could not download Python {wanted} — checking for a manually installed copy…\n"
        ));
        if let Some(p) = find() {
            if let Some(v) = verify_exact(&p) {
                emit(&format!(
                    "[*] Using existing Python {wanted}: {p} ({v}) — setup.py will reuse it.\n"
                ));
                return Ok(p);
            }
        }
    ...
    if !dl_ok {
        ...
        if reinstalled {
            ...
            let (retry_ok, _) =
                run_capture(app, &emit, uv_bin.as_str(), &["python", "install", wanted]).await;
            if retry_ok {
                if let Some(p) = find() {
                    if let Some(v) = verify_exact(&p) {
                        emit(&format!(
                            "[*] Python {wanted} ready after uv reinstall: {p} ({v})\n"
                        ));
                        return Ok(p);
                    }
                }
    ...
    if dl_ok {
        ...
        let (re_ok, _) = run_capture(
            app,
            &emit,
            uv_bin.as_str(),
            &["python", "install", "--reinstall", wanted],
        )
        .await;
        if !re_ok {
            ...
            let _ = run_capture(
                ...
                &emit,
                uv_bin.as_str(),
                &["python", "uninstall", wanted],
            )
            .await;
            let (ok2, _) =
                run_capture(app, &emit, uv_bin.as_str(), &["python", "install", wanted]).await;
            if !ok2 {
                return Err(diagnose_python_fail(wanted));
            }
        }
        if let Some(p) = find() {
            if let Some(v) = verify_exact(&p) {
...
fn diagnose_python_fail(wanted: &str) -> String {
    ...
    #[cfg(windows)]
    let path_hits: Vec<String> = silent_command("where")
        .arg("python")
        .output()
        .ok()
        .filter(|o| o.status.success())
        ...
        .unwrap_or_default();
    #[cfg(not(windows))]
    let path_hits: Vec<String> = silent_command("which")
        .args(["-a", "python3.11"])
        .output()
        .ok()
        .filter(|o| o.status.success())
    ...
    for fixed in [
        std::env::var("LOCALAPPDATA")
            .ok()
            .map(|a| format!("{a}\\Programs\\Python\\Python311\\python.exe")),
    ...
    .flatten()
    {
        if !cands.iter().any(|c| c.eq_ignore_ascii_case(&fixed)) {
            cands.push(fixed);
        }
    }
    #[cfg(not(windows))]
    for fixed in ["/usr/bin/python3.11", "/usr/local/bin/python3.11"] {
        if !cands.contains(&fixed.to_string()) {
            cands.push(fixed.into());
        }
    }
    ...
    let found_txt = if lines.is_empty() {
        "  (no Python 3.11 found on PATH, in registry spots, or via py launcher)".into()
    } else {
        lines.join("\n")
    };
...
pub fn python_preflight() -> serde_json::Value {
    ...
    let uv_bin = uv_command();
    let uv_ver = silent_command(uv_bin.as_str())
        .arg("--version")
        .output()
        .ok()
        .and_then(|o| {
    ...
    let path = silent_command(uv_bin.as_str())
        .args(["python", "find", &wanted])
        .output()
        .ok()
        .and_then(|o| {
    ...
    let runs = path.as_ref().is_some_and(|p| {
        silent_command(p)
            .arg("-c")
            .arg("import sys; print(sys.version)")
            .output()
    ...
    let downloads_blocked =
        std::env::var("UV_PYTHON_DOWNLOADS").is_ok_and(|v| v.eq_ignore_ascii_case("never"));
    // Where would `uv python install` put the interpreter? Explicit override,
    // else uv's default data dir (Windows: %APPDATA%\uv).
    let uv_data_dir = std::env::var("UV_PYTHON_INSTALL_DIR")
        .ok()
        .filter(|s| !s.is_empty())
        .or_else(|| std::env::var("APPDATA").ok().map(|a| format!("{a}\\uv")))
        .unwrap_or_default();
    let uv_data_free_gb = if uv_data_dir.is_empty() {
        None
    } else {
...
pub(crate) fn pinokio_root(repo: &Path) -> Option<PathBuf> {
    ...
    for _ in 0..3 {
        let dir = cur?;
        if dir.join("pinokio.js").exists()
            || dir.join("pinokio.json").exists()
            || dir.join(".pinokio").exists()
        {
...
pub fn classify_target() -> serde_json::Value {
    ...
    if entries.len() == count && entries.iter().all(|n| BENIGN.contains(&n.as_str())) {
        return no_target;
    }
    let has_wgp = repo.join("wgp.py").exists();
    let has_git = repo.join(".git").exists();
    // Upstream remote without spawning git: read .git/config.
    let remote: Option<String> = std::fs::read_to_string(repo.join(".git").join("config"))
        .ok()
        .and_then(|s| {
            s.lines()
    ...
    for ed in ["env_uv", "env_venv", "env_conda"] {
        ...
        envs.insert(
            ed.into(),
            serde_json::json!({"exists": true, "healthy": py.exists()}),
        );
    }
    let any_healthy = envs
        .values()
        .any(|e| e.get("healthy").and_then(|h| h.as_bool()).unwrap_or(false));
    ...
    let stale_tmp: Vec<String> = entries
        .iter()
        .filter(|n| n.starts_with(".wan2gp-clone-tmp-"))
        .cloned()
        .collect();
    let models = ["ckpts", "loras", "outputs"]
        .iter()
    ...
    let (verdict, hint) = if pinokio {
        ...
        (
            "ours_healthy",
            "A working Wan2GP install is already here — you can reuse it instead of reinstalling.",
        )
    ...
    for raw in chunk.split('\n') {
        ...
        let ev = if let Some(rest) = low.strip_prefix("downloading ") {
            ...
            if let Some((pkg, ver)) = stripped.split_once("==") {
                Some(
                    serde_json::json!({"phase": "package-installed", "pkg": pkg.trim(), "version": ver.trim()}),
                )
            } else {
                None
            }
...
fn install_failure_hint(tail: &str) -> String {
    ...
    {
        return "Disk full on the install drive — free space (50+ GB recommended) and retry."
            .into();
    }
    if low.contains("permission denied")
        || low.contains("access is denied")
        || low.contains("winerror 5")
    {
...
fn patch_therock_torch_cmd(repo: &std::path::Path, torch_cmd: &str) -> Result<(), String> {
    ...
    let raw =
        std::fs::read_to_string(&path).map_err(|e| format!("setup_config.json unreadable: {e}"))?;
    ...
    .map_err(|e| format!("setup_config.json unwritable: {e}"))?;
    Ok(())
}
...
pub(crate) fn forced_setup_profile(repo: &std::path::Path, plan_profile: &str) -> Option<String> {
    ...
    profiles
        ...
        .then(|| plan_profile.to_string())
}

/// Hook-mode argv: keep the interpreter-invocation prefix (`uv run ...
/// python`, `conda run -p <env> python`) and replace only the
/// `setup.py install --env X --auto` tail with the hook module (0.6.0
/// test failure: the first build passed the bare hook path to uv itself).
/// Pure + unit-tested.
fn hook_spawn_args(legacy: Vec<String>, hook: &str) -> Vec<String> {
    match legacy.iter().position(|a| a == "setup.py") {
        Some(i) => {
            let mut v = legacy[..i].to_vec();
            v.push(hook.to_string());
            v
        }
        None => vec![hook.to_string()],
    }
}

/// setup.py's chosen profile from its streamed stdout (fail-fast guard):
/// do_install_auto prints "Starting Automatic Install (Hardware Profile: X)".
/// The hook passes our key directly so a mismatch means upstream changed
/// the install routine under us — the caller kills the child on mismatch.
/// Pure + unit-tested.
fn parse_setup_profile_line(line: &str) -> Option<String> {
    let (_, rest) = line.split_once("Hardware Profile:")?;
...
/// setup.py driver hook (issue #15): run setup.py's --auto install with the
/// launcher's hardware verdict instead of setup.py's own detection (wmic.exe
/// is removed on current Windows 11 → Unknown → RTX_40 → full CUDA stack on
/// AMD boxes; its AMD name table misses RDNA 4 names; VRAM reads nvidia-smi
/// only → 8GB default). The hook imports setup.py as a module, applies the
/// verdict (WAN2GP_TAURI_* env) to its functions, and calls do_install_auto()
/// directly — upstream setup.py is NEVER modified, so upstream drift can't
/// silently re-enable the wrong stack: missing functions/keys exit 2 with a
/// clear message (fail-closed) instead of running a doomed install.
/// Written fresh on every hook-mode install; deleted when setup ends.
const SETUP_HOOK_FILENAME: &str = "wgp_tauri_setup_hook.py";
const SETUP_HOOK_SOURCE: &str = r##""""Tauri launcher setup hook (issue #15): run setup.py's --auto install with
the launcher's hardware verdict instead of setup.py's own detection.

setup.py detects the GPU via wmic.exe (removed on current Windows 11 ->
"Unknown" -> RTX_40 -> full CUDA stack on AMD boxes) and reads VRAM via
nvidia-smi only (-> 8GB default). The launcher passes its verdict through
WAN2GP_TAURI_* env vars; this hook imports setup.py as a module, applies the
verdict to its functions, and calls do_install_auto() directly.

Written fresh by the launcher on every install; deleted when setup ends.
Any structural problem exits 2 with a clear message so the launcher fails
fast instead of running a doomed 20-minute install."""
import os
import sys
import traceback
...
def _fail(msg):
    print("[!] Launcher setup hook: " + msg)
    sys.exit(2)


_PROFILE = _env("WAN2GP_TAURI_GPU_PROFILE")
_ENV_TYPE = _env("WAN2GP_TAURI_ENV_TYPE") or "uv"
_GPU_NAME = _env("WAN2GP_TAURI_GPU_NAME") or _PROFILE
_VRAM_RAW = _env("WAN2GP_TAURI_VRAM_GB")
_CONDA_PY = _env("WAN2GP_TAURI_CONDA_PY")

if not _PROFILE:
    _fail("WAN2GP_TAURI_GPU_PROFILE is unset - refusing to run without a forced profile.")

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
try:
    import setup as _setup
except Exception as e:
    _fail("could not import setup.py (%s). Upstream may have renamed it - report this with Copy diagnostics." % (e,))

for _fn in ("load_config", "do_install_auto", "get_system_specs", "ENV_TEMPLATES"):
    if not hasattr(_setup, _fn):
        _fail("setup.py no longer defines %s - upstream drift. Report this with Copy diagnostics." % (_fn,))

# conda-direct-pip: the stock conda template re-quotes args on Windows and
# corrupts URL-encoded wheel URLs (SpargeAttn %2B -> "Invalid build number").
# Drive pip with the env interpreter directly (conda keeps python.exe at the
# env root; the path is valid by the time the template is used - setup.py
# creates the env in step 1/3 before any pip call).
if _ENV_TYPE == "conda":
    if not _CONDA_PY:
        _fail("conda env needs WAN2GP_TAURI_CONDA_PY (env interpreter path).")
    try:
        _setup.ENV_TEMPLATES["conda"]["install"] = '"%s" -m pip install' % (_CONDA_PY,)
    except Exception as e:
        _fail("could not reroute the conda pip template (%s)." % (e,))

if _PROFILE.startswith("AMD"):
    _VENDOR = "AMD"
elif _PROFILE == "MPS":
    _VENDOR = "APPLE"
else:
    _VENDOR = "NVIDIA"
_setup.get_gpu_info = lambda: (_GPU_NAME, _VENDOR)

if _VRAM_RAW:
    try:
        _VRAM_GB = float(_VRAM_RAW)
    except Exception:
        _fail("WAN2GP_TAURI_VRAM_GB=%r is not a number." % (_VRAM_RAW,))
    _orig_specs = _setup.get_system_specs

    def _launcher_specs():
        # RAM: upstream's own logic (powershell, works on Win11) - keep it.
        # VRAM: upstream only knows nvidia-smi (AMD/Intel -> 8GB default).
        try:
            _ram, _ = _orig_specs()
        except Exception:
            _ram = 16.0
        return (_ram, _VRAM_GB)

    _setup.get_system_specs = _launcher_specs
    print("[*] Launcher-provided VRAM: %gGB (setup.py cannot read VRAM on this box)" % (_VRAM_GB,))

if hasattr(_setup, "inject_system_paths"):
    try:
        _setup.inject_system_paths()
    except Exception:
        pass

_cfg = _setup.load_config()
_profiles = _cfg.get("gpu_profiles", {}) if isinstance(_cfg, dict) else {}
if _PROFILE not in _profiles:
    _fail("profile %r is not in this setup_config.json (keys: %s) - refusing a KeyError install." % (_PROFILE, sorted(_profiles)))
print("[*] Launcher-forced GPU profile: %s (%s)" % (_PROFILE, _GPU_NAME))
try:
    _setup.do_install_auto(_ENV_TYPE, _cfg, _PROFILE)
except SystemExit:
    raise
except KeyError as e:
    _fail("setup_config.json lookup failed (%s) - upstream schema changed." % (e,))
except Exception:
    traceback.print_exc()
    sys.exit(1)
"##;

/// Write the setup hook into the cloned repo (fresh every hook-mode
/// install; upstream setup.py itself untouched). Stale copies removed first.
fn write_setup_hook(repo: &std::path::Path) -> Result<std::path::PathBuf, String> {
    let path = repo.join(SETUP_HOOK_FILENAME);
    let _ = std::fs::remove_file(&path);
    std::fs::write(&path, SETUP_HOOK_SOURCE).map_err(|e| format!("setup hook unwritable: {e}"))?;
    Ok(path)
}

/// Best-effort hook removal (every setup exit path + before write).
fn remove_setup_hook(repo: &std::path::Path) {
    let _ = std::fs::remove_file(repo.join(SETUP_HOOK_FILENAME));
}

/// Clear setup.py child env (process-scoped like the py-shim PATH prepend —
/// set before the setup child spawns, cleared after setup so later
...
fn clear_setup_child_env() {
    ...
    std::env::remove_var("WAN2GP_TAURI_GPU_NAME");
    std::env::remove_var("WAN2GP_TAURI_VRAM_GB");
    std::env::remove_var("WAN2GP_TAURI_ENV_TYPE");
    std::env::remove_var("WAN2GP_TAURI_CONDA_PY");
...
pub(crate) fn run_preflight_checks(
    ...
    match silent_command(tool_path("git").as_str())
        .args(["--version"])
        .output()
    {
        Ok(o) if o.status.success() => checks.push(PreflightCheck {
            id: "git",
            level: "ok",
            msg: format!(
                "git present ({})",
        ...
        _ => {
            ...
            checks.push(PreflightCheck {
                id: "git",
                level: "fail",
                msg: "git not found — install it (`winget install Git.Git`), then retry.".into(),
            });
        }
    }
    ...
    #[cfg(windows)]
    {
        let amds: Vec<String> = wmi_all_gpus()
            .into_iter()
        ...
        if !virtuals.is_empty() {
            if vendor == "unknown" || name.is_empty() {
                ...
                checks.push(PreflightCheck {
                    ...
                    level: "info",
                    msg: format!(
                        "ignoring virtual display adapter(s) ({}); using {name}.",
                        virtuals.join(" + ")
                    ),
                });
            }
    ...
    if let Ok(s) = std::fs::read_to_string(repo.join(".wan2gp-install-ok")) {
        ...
        #[cfg(windows)]
        let py_gone = !env_dir.join("Scripts\\python.exe").exists();
        #[cfg(not(windows))]
        let py_gone = !env_dir.join("bin/python").exists() && !env_dir.join("bin/python3").exists();
        if py_gone {
    ...
    if profile.starts_with("AMD") {
        if let Ok(raw) = std::fs::read_to_string(repo.join("wgp_config.json")) {
            let stale = serde_json::from_str::<serde_json::Value>(&raw)
                .ok()
                .and_then(|v| {
                    v.get("attention_mode")
                        .and_then(|a| a.as_str())
                        .map(str::to_string)
                })
                .is_some_and(|a| a.starts_with("sage"));
            if stale {
                checks.push(PreflightCheck {
                    id: "stale-config",
                    level: "info",
                    msg: "stale CUDA-era wgp_config.json on an AMD box — it will be regenerated."
                        .into(),
                });
            }
        }
        ...
        if let Some(c) = crate::amd::read_hsa_choice(repo) {
            checks.push(PreflightCheck {
                id: "hsa",
                level: "info",
                msg: format!("launch will use recorded HSA mode: {c:?}"),
            });
        }
    }
                ...
                if o.status.success() {
                    ...
                    checks.push(if covered {
                        PreflightCheck {
                            id: "av",
                            level: "ok",
                            msg: "Defender exclusion covers the install folder.".into(),
                        }
                    } else {
                        PreflightCheck {
                            id: "av",
                            level: "warn",
                            msg: av_exclusion_msg(vendor)
                                .unwrap_or("no Defender exclusion for the install folder.")
                                .into(),
                        }
                    });
                }
...
pub async fn install(
    ...
    emit(&format!(
        "[hw] GPU: {} ({}) — {} / {} — profile {}\n",
        plan["gpuName"].as_str().unwrap_or("?"),
        plan["vendor"].as_str().unwrap_or("?"),
        plan["cuda"].as_str().unwrap_or("?"),
        plan["torch"].as_str().unwrap_or("?"),
        plan["profile"].as_str().unwrap_or("?")
    ));
    if let Some(w) = plan["driverWarning"].as_str() {
        if !w.is_empty() {
            emit(&format!("[warn] {w}\n"));
        }
    }
    ...
    let emit_phase = |id: &str, label: &str, done: bool| {
        let _ = app.emit(
            "setup-phase",
            serde_json::json!({"id": id, "label": label, "done": done}),
        );
    };
    ...
    if repo.join("wgp.py").exists() {
        ...
        let needs_tmp =
            repo.exists() && std::fs::read_dir(&repo).is_ok_and(|mut it| it.next().is_some());
        if needs_tmp {
            ...
            emit(&format!(
                "[*] Target not empty — cloning into temp {}\n",
                tmp.display()
            ));
            use tauri_plugin_shell::ShellExt;
            ...
            while let Some(ev) = rx.recv().await {
                match ev {
                    CommandEvent::Stdout(b) => emit(&String::from_utf8_lossy(&b)),
                    CommandEvent::Stderr(b) => emit(&String::from_utf8_lossy(&b)),
                    _ => {}
                }
            }
            if !tmp.join("wgp.py").exists() {
                mutating_done();
            ...
            for e in std::fs::read_dir(&tmp).map_err(|e| e.to_string())? {
                ...
                if dst.exists() && name == ".git" {
                    let _ = std::fs::remove_dir_all(&dst);
                }
                let _ = std::fs::rename(e.path(), &dst).or_else(|_| {
                    if e.path().is_dir() {
                        fs_extra_fallback_copy_dir(&e.path(), &dst)
                    } else {
    ...
    emit(&format!(
        "[*] Installing env={env} via setup.py (streaming)…\n"
    ));
    ...
    #[cfg(windows)]
    let legacy_py = env_path.join("Scripts\\python.exe");
    #[cfg(not(windows))]
    let legacy_py = env_path.join("bin/python");
    let py_exe = resolve_env_python(&repo, &env_path.to_string_lossy()).unwrap_or(legacy_py);
    let marked_same_env = std::fs::read_to_string(&marker)
        .ok()
    ...
    if marked_same_env || torch_probe(&py_exe).is_some() {
        ...
        match smoke_verify(&py_exe, &repo) {
            Ok(line) => {
                emit(&format!(
                    "[*] Previous install verified ({line}) — skipping re-download…\n"
                ));
            ...
            Err(e) => {
                ...
                let mut msg = String::from(
                    "[!] Previous env is damaged (torch won't import) — rebuilding clean…\n",
                );
                let low = e.to_lowercase();
                if low.contains("dll")
                    || low.contains("winerror 126")
                    || low.contains("0xc0000135")
                    || low.contains("error_mod_not_found")
                {
    ...
    if let Ok(rd) = std::fs::read_dir(&repo) {
        for e in rd.flatten() {
            ...
            if n.starts_with(".wan2gp-clone-tmp-") {
                ...
                emit(&format!(
                    "[*] Removed leftover {n} from an interrupted install.\n"
                ));
            }
        }
    }
    ...
    #[cfg(windows)]
    if env == "venv" {
        let shim_ok = silent_command(tool_path("py").as_str())
            .args(["-3.11", "-c", "import sys"])
            .output()
            .is_ok_and(|o| o.status.success());
        if !shim_ok {
            let wanted = pinned_python_wanted();
            match ensure_uv_python(&app, &emit, &wanted).await {
                Ok(uvpy) => {
                    ...
                    saved_path = Some(old);
                    if silent_command(tool_path("py").as_str())
                        .args(["-3.11", "-c", "import sys"])
                        .output()
                        .is_ok_and(|o| o.status.success())
                    {
    ...
    let hook_path: Option<std::path::PathBuf> = if let Some(key) = forced_profile.as_deref() {
        ...
        std::env::set_var(
            "WAN2GP_TAURI_GPU_NAME",
            plan["gpuName"].as_str().unwrap_or(key),
        );
        emit(&format!(
            "[*] setup.py will run with forced GPU profile: {key}\n"
        ));
        match plan.get("vramGb").and_then(|v| v.as_f64()) {
            Some(mb) if mb >= 2048.0 => {
                std::env::set_var(
                    "WAN2GP_TAURI_VRAM_GB",
                    format!("{}", (mb / 1024.0).round() as u64),
                );
                emit(&format!(
                    "[*] setup.py will run with launcher VRAM: {}GB\n",
                    (mb / 1024.0).round() as u64
                ));
            }
            // No usable figure: hook leaves VRAM to setup.py (nvidia-smi
            // or its 8GB default) — same fallback as before the hook.
            _ => {
                std::env::remove_var("WAN2GP_TAURI_VRAM_GB");
                emit("[!] No launcher VRAM figure — setup.py falls back to its own detection.\n");
            }
        }
        std::env::set_var("WAN2GP_TAURI_ENV_TYPE", env.as_str());
        if env.as_str() == "conda" {
            // conda-direct-pip: `conda run` re-quotes args on Windows and
            // corrupts URL-encoded wheel URLs (SpargeAttn %2B). The hook
            // reroutes the template to the env interpreter directly — the
            // path is valid by the time the template is used (setup.py
            // creates the env in step 1/3 before any pip call).
            #[cfg(windows)]
            let cpy = env_path.join("python.exe");
            #[cfg(not(windows))]
            let cpy = env_path.join("bin/python");
            std::env::set_var("WAN2GP_TAURI_CONDA_PY", cpy.to_string_lossy().to_string());
        } else {
            std::env::remove_var("WAN2GP_TAURI_CONDA_PY");
        }
        match write_setup_hook(&repo) {
            Ok(p) => {
                emit("[*] setup.py launcher hook ready (profile + VRAM + conda pip via module import — setup.py itself untouched).\n");
                Some(p)
            }
            Err(e) => {
                #[cfg(windows)]
                if let Some(old) = saved_path.clone() {
                    std::env::set_var("PATH", old);
                }
                clear_setup_child_env();
                mutating_done();
                return Err(format!(
                    "Could not stage the setup hook ({e}) — refusing to run setup.py unpatched."
                ));
            }
        }
    } else {
    ...
    // Verdict env for the setup child below (WAN2GP_TAURI_* set above;
    // unknown keys unset so the hook path stays off and setup.py falls
    // back to its own detection). Process-scoped like the py-shim PATH
    // prepend; cleared after setup.
    ...
    // run setup.py with the env's python (hardware-aware: setup.py reads setup_config.json + GPU)
    {
        let (py, args): (String, Vec<String>) = if env.as_str() == "conda" {
            ...
            if conda_bin != "conda" {
                for ch in [
                    ...
                    let ok = silent_command(conda_bin.as_str())
                        ...
                        .is_ok_and(|o| o.status.success());
                    if ok {
                        emit(&format!("[*] Anaconda ToS accepted: {ch}\n"));
                    } else {
                        emit(&format!("[!] conda ToS accept failed for {ch} (old conda without enforcement, or offline) — continuing; channel ops may refuse.\n"));
                    }
                }
            }
            if resolve_env_python(&repo, &env_path.to_string_lossy()).is_some() {
                ...
                let runs = silent_command(syspy.as_str())
                    .arg("-c")
                    .arg("import sys")
                    .output()
                    .is_ok_and(|o| o.status.success());
            ...
            let (py_bin, via_uv_run) = if p.exists() {
        ...
        // Hook mode (issue #15): the verdict travels via WAN2GP_TAURI_*
        // env, not argv — run the hook module instead of setup.py, keeping
        // the interpreter prefix (uv/conda run). Legacy path (INTEL_XPU /
        // CPU) keeps setup.py's own `install --env X --auto` detection.
        let hook_mode = hook_path.is_some();
        let args: Vec<String> = match hook_path.as_ref() {
            Some(hp) => hook_spawn_args(args, &hp.to_string_lossy()),
            None => args,
        };
        ...
        for attempt in 1..=2 {
            ...
            let mut do_phase = |id: &str, label: &str| {
            ...
            let mut done_phase = |id: &str, label: &str| {
                if phases_done.insert(id.to_string()) {
                    let _ = app.emit(
                        "setup-phase",
                        serde_json::json!({"id": id, "label": label, "done": true}),
                    );
                }
            };
            if attempt == 2 {
                ...
                .spawn()
            {
                ...
                Err(e) => {
                    ...
                    clear_setup_child_env();
                    remove_setup_hook(&repo);
                    mutating_done();
                    return Err(format!("setup.py failed to start ({e})"));
                }
            };
            // Option dance: kill() consumes the child; take() keeps the
            // borrow checker happy on the guard path below.
            ...
            while let Some(ev) = rx.recv().await {
                match ev {
                    CommandEvent::Stdout(b) | CommandEvent::Stderr(b) => {
                        ...
                        // upstream changed the install routine under us —
                        // kill the child before the wrong stack downloads.
                        if hook_mode {
                            if let Some(f) = forced_profile.as_deref() {
                                for ln in txt.lines() {
                                    if let Some(reported) = parse_setup_profile_line(ln) {
                                        if reported != f {
                                            emit(&format!("[!] setup.py chose profile {reported} (launcher forced {f}) — aborting before the wrong stack downloads.\n"));
                                            ...
                                            remove_setup_hook(&repo);
                                            mutating_done();
                                            return Err(format!("Aborted: setup.py ignored the forced profile {f} and picked {reported} (upstream drift — its install routine changed). Copy diagnostics (System → Troubleshooting) and report this; do NOT retry blindly."));
                                        }
                                        emit(&format!(
                                            "[✓] setup.py confirmed profile: {reported}\n"
                                        ));
                                    }
                                }
                            }
                        ...
                        if low.contains("[1/3]") || low.contains("preparing environment") {
                            do_phase("venv", "Create Python virtual environment");
                        }
                        if low.contains("[2/3]")
                            || low.contains("installing torch")
                            || low.contains("download.pytorch.org")
                        {
                            done_phase("venv", "Create Python virtual environment");
                            do_phase("torch", "Install PyTorch + CUDA");
                        }
                        if low.contains("[3/3]")
                            || low.contains("installing requirements")
                            || low.contains("-r requirements")
                        {
                            done_phase("torch", "Install PyTorch + CUDA");
                            do_phase("reqs", "Install Python dependencies");
                        }
                        if let Some(h) = low.split("[*] install").nth(1) {
                            // Component header — check flash/sparge before sage
                            // ("spas-sage-attn" contains "sage").
                            if h.starts_with("flash")
                                || h.contains("spas-sage")
                                || h.contains("sparge")
                            {
                                do_phase("flash", "Install Flash Attention");
                            } else if h.contains("sage") {
                                do_phase("sage", "Install Sage Attention kernel");
                            } else if h.contains("triton") {
                                do_phase("triton", "Install Triton compiler");
                            } else if h.contains("torch") || h.contains("cuda") {
                                done_phase("venv", "Create Python virtual environment");
                                do_phase("torch", "Install PyTorch + CUDA");
                            } else if h.contains("nunchaku")
                                || h.contains("gguf")
                                || h.contains("kernel")
                                || h.contains("lightx2v")
                            {
                                do_phase("kernels", "Install GPU kernels (nunchaku/GGUF)");
                            } else if h.contains("requirement") {
                                done_phase("torch", "Install PyTorch + CUDA");
                                do_phase("reqs", "Install Python dependencies");
                            }
                        }
                        if low.contains("downloading triton") || low.contains("+ triton") {
                            do_phase("triton", "Install Triton compiler");
                        }
                        if low.contains("downloading sageattention")
                            || low.contains("+ sageattention")
                        {
                            do_phase("sage", "Install Sage Attention kernel");
                        }
                        if low.contains("downloading flash") || low.contains("+ flash") {
                            do_phase("flash", "Install Flash Attention");
                        }
                        if low.contains("downloading nunchaku") || low.contains("+ nunchaku") {
                            do_phase("kernels", "Install GPU kernels (nunchaku/GGUF)");
                        }
                        // Completions: uv's "+ <pkg>==" resolved lines (each arrives
                        // separately from "Installed 1 package", so single tokens).
                        if low.contains("+ torch==") {
                            done_phase("torch", "Install PyTorch + CUDA");
                        }
                        if low.contains("+ triton") {
                            done_phase("reqs", "Install Python dependencies");
                            done_phase("triton", "Install Triton compiler");
                        }
                        if low.contains("+ sageattention") {
                        ...
                        if low.contains("+ llamacpp") || low.contains("+ lightx2v") {
            ...
            mutating_done();
            return Err(format!(
                "Install failed (setup.py exited code {code}). {hint}"
            ));
        ...
        for (id, label) in [
            ...
            ("torch", "Install PyTorch + CUDA"),
            ("reqs", "Install Python dependencies"),
            ("triton", "Install Triton compiler"),
            ("sage", "Install Sage Attention kernel"),
            ("flash", "Install Flash Attention"),
            ("kernels", "Install GPU kernels (nunchaku/GGUF)"),
        ] {
            let _ = app.emit(
                "setup-phase",
                serde_json::json!({"id": id, "label": label, "done": true}),
            ...
            let probe_modes = |winner: &mut Option<crate::amd::HsaChoice>,
                ...
                for (label, hsa) in &modes {
                    ...
                    match crate::amd::run_compute_probe(&smoke_py, hsa.as_deref()) {
                        ...
                        Err(e) => {
                            emit(&format!(
                                "[!] GPU compute probe (HSA {label}) failed: {e}\n"
                            ));
                            *last_err = e;
                        }
                    }
            ...
            if winner.is_none() && (env == "uv" || env == "venv") {
                if let Some((_, staging)) = &amd_cmds {
                    ...
                    let (prog, mut args): (String, Vec<String>) = if env == "uv" {
                        ...
                        #[cfg(windows)]
                        let py = env_path.join("Scripts\\python.exe");
                        #[cfg(not(windows))]
                        let py = env_path.join("bin/python3");
                        (
    ...
    let _ = atomic_write(
        &home_dir().join(".wan2gp-tauri-installed"),
...
pub async fn reinstall(
        ...
        .and_then(|m| m.as_array())
    {
        for mv in moves {
            ...
            if low(from) == low(&repo.to_string_lossy()) {
                emit(&format!("[!] Skipping move of the repo itself: {from}\n"));
                continue;
            }
            if low(to).starts_with(&low(&repo.to_string_lossy())) {
                emit(&format!(
                    "[!] Skipping move into the wiped folder (would be deleted): {to}\n"
                ));
                continue;
            }
            emit(&format!(
                "[*] Moving models out before wipe: {from} → {to}\n"
            ));
            match crate::system::move_path_inner(&app, Path::new(from), Path::new(to)).await {
                ...
                Err(e) => {
                    mutating_done();
                    return Err(format!(
                        "Could not move models ({e}). Wipe aborted — nothing deleted."
                    ));
                }
            }
        }
    ...
    if !want_backup {
        ...
        for sub in ["plugins", "finetunes", "deepy_sessions"] {
            let s = repo.join(sub);
            if s.exists() {
                let d = backup.join(sub);
                let _ = silent_command("xcopy")
                    .args([
                        "/E",
                        "/I",
    ...
    if repo.exists() {
        ...
        let trash = repo.with_file_name(format!(
            "{}.trash-{}",
        ...
        if let Ok(ents) = std::fs::read_dir(&repo) {
                ...
                if std::fs::rename(&src, &dst).is_err() {
                    ...
                    #[cfg(windows)]
                    {
                        let _ = silent_command("cmd")
                            .args(["/C", &format!("attrib -R /S /D \"{}\"", src.display())])
                            .output();
                    }
                    if std::fs::rename(&src, &dst).is_err() {
                        let mut moved = false;
                        for _ in 0..5 {
                            if std::fs::remove_dir_all(&src).is_ok()
            ...
            for _ in 0..5 {
                ...
                {
                    let _ = silent_command("cmd")
                        .args(["/C", &format!("attrib -R /S /D \"{}\"", git.display())])
                        .output();
                }
            ...
            std::thread::spawn(move || {
                let _ = std::fs::remove_dir_all(&trash_clone);
            });
            if ok {
                emit("[*] Old installation moved to trash (kept .electron) — fresh install starting...\n");
            }
        }
        if !ok {
    ...
    mutating_done();
    Ok(serde_json::json!({"ok": true, "success": true, "movedModels": moved_models}))
}
...
pub async fn restore_backup(app: tauri::AppHandle) -> Result<serde_json::Value, String> {
    ...
    if !backup.exists() {
        return Ok(serde_json::json!({"ok": true, "success": true, "restored": []}));
    }
    ...
    for sub in ["plugins", "finetunes", "deepy_sessions"] {
        ...
        if let Ok(rd) = std::fs::read_dir(&s) {
            for e in rd.flatten() {
                ...
                if dst.exists() {
                    continue;
                } // fresh clone's own copy wins
                if std::fs::rename(e.path(), &dst).is_ok() {
                    restored.push(format!("{sub}/{name}"));
                }
            }
        }
    }
    ...
    if bc.exists() {
        if !repo.join("wgp_config.json").exists() {
            if std::fs::copy(&bc, repo.join("wgp_config.json")).is_ok() {
                restored.push("wgp_config.json".into());
            }
    ...
    emit(&format!(
        ...
        if restored.is_empty() {
            "nothing new (fresh defaults kept)".into()
        } else {
            restored.join(", ")
        }
    ));
...
pub async fn uninstall(
    ...
    if !repo.exists() {
        mutating_done();
        return Err("Wan2GP not installed".into());
    }
    if let Some(where_) = pinokio_root(&repo) {
        mutating_done();
    ...
    let keep = match options
        .as_ref()
        .and_then(|o| o.get("keepModels"))
        .and_then(|k| k.as_bool())
    {
        ...
        None => {
            if !app
                .dialog()
                .message(
                    "Uninstall Wan2GP?\n\nRemoves the app, its Python environment and packages.",
                )
            ...
            app.dialog()
                .message("Keep your downloaded files? (OK = keep, Cancel = delete)")
                .title("Keep models?")
                .kind(MessageDialogKind::Info)
                .blocking_show()
        }
    ...
    if keep {
        ...
        for key in ["checkpoints", "loras", "output"] {
                ...
                .filter(|s| !s.is_empty() && *s != ".")
            {
                ...
                if abs.starts_with(&repo) && abs.exists() {
                    keep_dirs.push(abs);
                }
            }
        }
    ...
    mutating_done();
    Ok(
        serde_json::json!({"success": true, "keptFiles": keep && !kept_paths.is_empty(), "keptPaths": kept_paths, "leftoverFolder": leftover}),
    )
}
#[tauri::command]
pub async fn sync_kernels(app: tauri::AppHandle) -> Result<serde_json::Value, String> {
    ...
    let cfg: serde_json::Value = if cfg_path.exists() {
        ...
        if let Ok(o) = silent_command("curl").args(["-sL", url]).output() {
            if o.status.success() {
        ...
        if raw.trim().is_empty() || !raw.trim().starts_with('{') {
            if let Ok(o) = silent_command("powershell")
                .args([
                    "-NoProfile",
                    "-Command",
                    &format!("(Invoke-WebRequest -Uri '{url}' -UseBasicParsing).Content"),
                ])
                .output()
            {
                if o.status.success() {
        ...
        if raw.trim().is_empty() || !raw.trim().starts_with('{') {
            mutating_done();
            return Err("setup_config.json missing and remote fetch failed".into());
        }
        emit_log(
            "[*] using remote setup_config.json (origin/main) — deepbeepmeep's wanted wheels\n",
        );
        serde_json::from_str(&raw).map_err(|e| e.to_string())?
    };
    // log commit + gguf version (a058daf)
    {
        let head = silent_command(tool_path("git").as_str())
    ...
    if ["RTX_30", "RTX_40", "RTX_50"].contains(&profile.as_str()) {
        ...
        if !all_kernels
            .iter()
            .any(|k| k.as_str() == Some("sage") || k.as_str() == Some("sageattention"))
        {
        ...
        if let Some(name) = k.as_str() {
            ...
            if name == "sage" || name == "sageattention" {
                if sage_safe && url.contains("sageattention-2.2.0+cu130torch2.9.0andhigher.post4") {
                    ...
                    && url.contains("sageattention-2.2.0+cu130torch2.10.0andhigher.post6")
                {
...
pub(crate) fn fs_extra_fallback_copy_dir(src: &Path, dst: &Path) -> Result<(), String> {
    ...
    for e in std::fs::read_dir(src).map_err(|e| e.to_string())? {
        ...
        if s.is_dir() {
            fs_extra_fallback_copy_dir(&s, &d)?;
        } else {
...
fn refresh_path_from_registry() {
    ...
    for (hk, sub) in [
        ...
        let Ok(o) = silent_command("reg")
            .args(["query", &target, "/v", "Path"])
            .output()
        else {
            continue;
        };
        ...
        for line in String::from_utf8_lossy(&o.stdout).lines() {
            ...
            let Some(pos) = low.find("reg_expand_sz").or_else(|| low.find("reg_sz")) else {
                continue;
            };
            let after_type = line[pos..]
                .split_whitespace()
                .skip(1)
                .collect::<Vec<_>>()
                .join(" ");
            ...
            while i < expanded.len() {
                ...
                let a = a + i;
                let Some(b) = expanded[a + 1..].find('%') else {
                    break;
                };
                let b = b + a + 1;
...
async fn run_live(app: &tauri::AppHandle, prog: &str, args: Vec<String>) -> bool {
    ...
    let emit = |msg: String| {
        crate::base::push_log(&msg, "setup");
        let _ = app.emit("setup-output", msg);
    };
    ...
    while let Some(ev) = rx.recv().await {
        match ev {
            CommandEvent::Stdout(b) | CommandEvent::Stderr(b) => {
                emit(String::from_utf8_lossy(&b).to_string())
            }
            CommandEvent::Terminated(p) => {
                code = p.code;
            }
...
fn probe_tool(tool: &str) -> bool {
    ...
    match tool {
        ...
        "uv" => {
            on_path("uv")
                || owned_uv().is_some()
                || exists(format!("{home}\\.local\\bin\\uv.exe"))
                || exists(format!("{home}\\.cargo\\bin\\uv.exe"))
        }
        "python" => {
            tool_usable("python")
                || tool_usable("py")
                || known_tool_path("python").is_some()
                || known_tool_path("py").is_some()
        }
        "conda" => {
            on_path("conda")
                || known_tool_path("conda").is_some()
...
fn verify_sha256(path: &str, expected_hex: &str, emit: &impl Fn(String)) -> bool {
    ...
    let mut file = match std::fs::File::open(path) {
        Ok(f) => f,
        Err(e) => {
            emit(format!("[!] checksum: cannot read {path}: {e}\n"));
            return false;
        }
    };
...
async fn latest_git_release() -> Option<(String, String)> {
    ...
    if base.is_empty() || !n.chars().all(|c| c.is_ascii_digit()) {
        return None;
    }
...
async fn official_fallback(app: &tauri::AppHandle, tool: &str) -> bool {
    ...
    match tool {
        "uv" => {
            ...
            run_live(
                ...
                vec![
                    ...
                    "Bypass".into(),
                    "-Command".into(),
                    "& { iwr -useb https://astral.sh/uv/install.ps1 | iex }".into(),
                ],
            )
            .await
        }
        "git" => {
            ...
            let mut sha: Option<&str> = if ver.as_str() == FALLBACK_VER {
                Some(FALLBACK_SHA)
            } else {
                None
            };
            emit("[*] Downloading Git for Windows (~120 MB)…\n".into());
            if !run_live(app, "powershell", download(&url, &dest)).await
                || !std::path::Path::new(&dest).exists()
            {
                if ver.as_str() != FALLBACK_VER {
                    // Live asset name changed or offline — retry the pinned release.
                    emit(format!(
                        "[!] Git {ver} download failed — retrying pinned Git {FALLBACK_VER}…\n"
                    ));
            ...
            match sha {
                ...
                None => emit(format!(
                    "[!] No pinned checksum for Git {ver} — trusting TLS.\n"
                )),
                _ => {}
            }
        ...
        "python" => {
            ...
            if !verify_sha256(&dest, SHA, &emit) {
                return false;
            }
            emit("[*] Installing silently — a couple of minutes…\n".into());
            run_live(
        ...
        "conda" => {
            ...
            run_live(
                ...
                &dest,
                vec![
                    "/InstallationType=JustMe".into(),
                    "/RegisterPython=0".into(),
                    "/S".into(),
                    format!("/D={home}\\Miniconda3"),
                ],
            )
            .await
        }
...
#[cfg(windows)]
async fn install_prerequisite_windows(
    app: tauri::AppHandle,
    tool: String,
) -> Result<serde_json::Value, String> {
    let emit = |msg: &str| {
    ...
    emit(&format!(
        "[*] Installing {tool} via winget (silent, a few minutes)…\n"
    ));
    ...
    if !ok {
        emit(&format!(
            "[!] winget failed or is missing — falling back to the official {tool} installer…\n"
        ));
        ok = official_fallback(&app, &tool).await;
    }
    if !ok {
        return Err(format!("{tool} install failed — see output above (needs network; some packages need admin approval; check antivirus)"));
    }
    ...
    if tool == "python" {
        let wanted = pinned_python_wanted();
        if !python_matches_pin("python", &[], &wanted)
            && !python_matches_pin("py", &["-3.11"], &wanted)
        {
            if probe_tool(&tool) {
    ...
    emit(&format!(
        "[✓] {tool} installed — restart the launcher so PATH picks it up.\n"
    ));
...
const DLSS5_FILES: &[(&str, &str, &str, &str)] = &[
    (
        ...
        "workers",
        "Workers v1.1.3",
        "F8E2967912E5D596E8E36049370487B83620B0CB5845937B681CF835BAFC6D0B",
    ),
    (
        "host/nvngx.dll",
        "workers",
        "Workers v1.1.3",
        "58191F4D38288C6BFBDA47EF56911D32052A9789E65714F4583F426E01464638",
    ),
    (
        "dlssg/dlssg-worker.exe",
        "workers",
        "Workers v1.1.3",
        "D93084633E0AAB4A08C43A5EE240176716EF73D87F06F35C2293509FBFC8BD00",
    ),
    (
        "host/dxgi.dll",
        "reshade",
...
pub fn dlss5_status() -> serde_json::Value {
    ...
    let present = files
        .iter()
        .filter(|f| f["installed"].as_bool().unwrap_or(false))
        .count();
...
fn dlss5_classify(app: &tauri::AppHandle, chunk: &str) {
    let pkg = |name: &str| -> &str {
        ...
        if n.contains("workers") {
            "workers"
        } else if n.contains("reshade") {
            "reshade"
        } else if n.contains("renodx") {
            "renodx"
        } else if n.contains("dlssnr") {
            "dlssnr"
        } else if n.contains("frame generation") {
            "dlssg"
        } else if n.contains("super resolution") {
            "dlss"
        } else {
            "other"
        }
    ...
    for raw in chunk.split('\n') {
        ...
        let ev = if let Some(name) = t.strip_prefix("Downloading ") {
            Some(
                serde_json::json!({"phase": "downloading", "pkg": pkg(name), "label": name.trim()}),
            )
        ...
        if let Some(ev) = ev {
            let _ = app.emit("dlss5-progress", ev);
        }
    }
}

#[tauri::command]
pub async fn install_dlss5(
    app: tauri::AppHandle,
    force: bool,
) -> Result<serde_json::Value, String> {
    ...
    {
        mutating_done();
        return Err("DLSS5 is Windows-only".into());
    }
    let repo = get_repo_dir();
    let emit = |msg: &str| {
        crate::base::push_log(msg, "setup");
    ...
    if !ps1.exists() {
        mutating_done();
        return Err("install_dlss5.ps1 not found — update Wan2GP first".into());
    }
    ...
    let installed = st
        ...
        .unwrap_or(false);
    mutating_done();
    if complete && !spawn_err {
        return Ok(serde_json::json!({"ok": true, "success": true, "complete": true}));
    }
    if installed {
        return Ok(
            serde_json::json!({"ok": true, "success": true, "complete": false, "hint": "Partial install — see console; rerun with Force if files conflict."}),
        );
    }
...
#[cfg(test)]
mod setup_hook_tests {
    use super::{
        forced_setup_profile, hook_spawn_args, parse_setup_profile_line, remove_setup_hook,
        write_setup_hook, SETUP_HOOK_SOURCE,
    };
    fn cfg_dir(tag: &str, profiles: &[&str]) -> std::path::PathBuf {
        let d = std::env::temp_dir().join(format!("wgp-setup-hook-{tag}-{}", std::process::id()));
        ...
        let keys = profiles
            ...
            .join(",");
        std::fs::write(
            d.join("setup_config.json"),
            format!("{{\"gpu_profiles\": {{{keys}}}}}"),
        )
        .unwrap();
        d
    }
    #[test]
    fn forced_profile_follows_cloned_config_keys() {
        // Runtime allowlist: whatever the CLONED setup_config.json defines
        // (upstream renames can't strand us on a stale hardcoded list).
        let d = cfg_dir("keys", &["RTX_40", "AMD_GFX1201", "MPS"]);
        assert_eq!(
            forced_setup_profile(&d, "AMD_GFX1201").as_deref(),
            Some("AMD_GFX1201")
        );
        assert_eq!(
            forced_setup_profile(&d, "RTX_40").as_deref(),
            Some("RTX_40")
        );
        ...
        for k in ["INTEL_XPU", "CPU", "", "RTX_99"] {
            assert_eq!(forced_setup_profile(&d, k), None, "{k}");
            assert_eq!(forced_setup_profile(&d2, k), None, "{k}");
        }
        // Unreadable / invalid config → no forcing, never a guess.
        let missing =
            std::env::temp_dir().join(format!("wgp-setup-hook-absent-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&missing);
        assert_eq!(forced_setup_profile(&missing, "RTX_40"), None);
        std::fs::write(d.join("setup_config.json"), "not json").unwrap();
        assert_eq!(forced_setup_profile(&d, "RTX_40"), None);
        let _ = std::fs::remove_dir_all(&d);
    }
    #[test]
    fn hook_argv_keeps_interpreter_prefix() {
        // Fresh-uv installs drive setup via `uv run --with setuptools
        // python …` — the hook replaces only the `setup.py …` tail
        // (0.6.0 test failure: bare hook path reached uv → exit 2).
        let hook = "C:\\Wan2GP\\wgp_tauri_setup_hook.py";
        assert_eq!(
            hook_spawn_args(
                vec![
                    "run".into(),
            ...
            vec!["run", "--with", "setuptools", "python", hook]
        );
        assert_eq!(
            hook_spawn_args(
                vec![
        ...
        // Unexpected shape without a setup.py element: hook alone
        // (it takes everything via env).
        assert_eq!(hook_spawn_args(vec!["--weird".into()], hook), vec![hook]);
    }
    #[test]
    fn profile_line_parses_setup_output() {
        assert_eq!(
            parse_setup_profile_line(
                "[*] Starting Automatic Install (Hardware Profile: AMD_GFX1201)..."
            )
            .as_deref(),
            Some("AMD_GFX1201")
        );
        assert_eq!(
            parse_setup_profile_line("Hardware Detected: Unknown (UNKNOWN)").as_deref(),
            None
        );
        assert_eq!(
            parse_setup_profile_line("downloading torch").as_deref(),
            None
        );
        assert_eq!(
            parse_setup_profile_line("Hardware Profile: ").as_deref(),
            None
        );
    }
    #[test]
    fn hook_source_is_valid_python_with_fail_closed_contract() {
        // Contract: drives do_install_auto via module import (never edits
        // setup.py), reroutes conda pip, validates the profile key, and
        // exits 2 on any structural surprise.
        for token in [
            "import setup as _setup",
            "do_install_auto",
            "get_system_specs",
            "ENV_TEMPLATES",
            "WAN2GP_TAURI_GPU_PROFILE",
            "WAN2GP_TAURI_VRAM_GB",
            "WAN2GP_TAURI_ENV_TYPE",
            "WAN2GP_TAURI_CONDA_PY",
            "-m pip install",
            "gpu_profiles",
            "sys.exit(2)",
        ] {
            assert!(SETUP_HOOK_SOURCE.contains(token), "hook missing {token}");
        }
        assert!(
            !SETUP_HOOK_SOURCE.contains("conda run"),
            "hook must not shell out to conda run"
        );
        // Must actually parse (never ship a SyntaxError into an install).
        // python may be absent — skip then.
        let check = std::env::temp_dir().join(format!("wgp-hook-ast-{}.py", std::process::id()));
        std::fs::write(&check, SETUP_HOOK_SOURCE).unwrap();
        let py = super::tool_path("python");
        let ok = super::silent_command(py.as_str())
            .args([
                "-c",
        ...
        assert!(ok, "hook source failed ast.parse");
    }
    #[test]
    fn hook_write_remove_round_trip() {
        let d = cfg_dir("hook", &["RTX_40"]);
        let p = write_setup_hook(&d).unwrap();
        assert_eq!(std::fs::read_to_string(&p).unwrap(), SETUP_HOOK_SOURCE);
        // Rewrite overwrites stale copies (fresh every install).
        std::fs::write(&p, "stale").unwrap();
        write_setup_hook(&d).unwrap();
        assert_eq!(std::fs::read_to_string(&p).unwrap(), SETUP_HOOK_SOURCE);
        remove_setup_hook(&d);
        assert!(!p.exists(), "hook not removed");
        // Removing twice is fine (best-effort on every exit path).
        remove_setup_hook(&d);
        let _ = std::fs::remove_dir_all(&d);
    }
}
...
mod preflight_tests {
    ...
    fn healthy_amd_box_passes_driver_and_vram() {
        ...
        assert!(
            !fatal || level(&checks, "git") == Some("ok"),
            "only git may fail, nothing fabricated"
        );
    ...
    fn stale_state_is_reported() {
        ...
        std::fs::write(
            repo.join("wgp_config.json"),
            r#"{"attention_mode": "sage2"}"#,
        )
        .unwrap();
...
mod owned_uv_tests {
    ...
    fn receiptless_copy_is_not_owned_but_runnable() {
        ...
        let Some(real) = path_uv_exe() else {
            return;
        };
    ...
    fn marker_plus_working_binary_is_owned() {
        ...
        assert!(std::process::Command::new(&owned)
            .arg("--version")
            .output()
            .is_ok_and(|o| o.status.success()));
        let _ = std::fs::remove_dir_all(&data);
...
mod tool_path_tests {
    ...
    fn candidate_tables_cover_stock_layouts() {
        ...
        assert!(conda.iter().any(|p| p
            .to_string_lossy()
            .contains("Miniconda3\\Scripts\\conda.exe")));
        // .bat is never offered as a spawn target (not a process image).
        assert!(!conda
            .iter()
            .any(|p| p.extension().is_some_and(|e| e == "bat")));
        assert_eq!(
            tool_candidates("py", home),
