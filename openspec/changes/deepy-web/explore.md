...
- Fields: `deepy_enabled` (0/1), `deepy_type` (`zero`|`prime`), `enhancer_enabled` (1/2 disabled; 3/4/5 zero; 5 for local-qwen38 Prime), `llm_engines.deepy` + `prompt_enhancer:same_as_deepy` + `profiles.<x>.executable` (literal exe name, never absolute path) + `base_url` for opencode.
...
- Reader: `deepy_status()` returns `{mode, deepyEnabled, deepyType, currentEngine, promptEnhancer, enhancerEnabled, engines}`; UI `refreshDeepy()` (~`src/app.js:6040`) renders mode radios + enhancer + engine dots from `llm_engines_list`.
