...
function renderDlss5Progress() {
  ...
  box.innerHTML = _dlss5Pkgs.map(p => {
    ...
    if (s.phase === 'downloading') return `<div class="spec-row"><span class="spec-label"><span>…</span> ${p.label}</span><span class="spec-value">downloading…</span></div>`
    const ok = s.phase === 'verified'
    const icon = ok ? '<span class="dot-ok">●</span>' : '<span style="color:#F87171">●</span>'
    const note = (ok ? '✓ SHA ' : 'SHA ') + sha.slice(0, 12) + '… ' + (ok ? '' : '— not installed')
