...
(() => {
    ...
    window.__syncNativeBounds = (logIt) => {
        try {
            ...
            if (w < 10 || h < 10) return;
            if (logIt)
                console.log("[embed] native bounds", {
                    x: Math.round(x),
                    y: Math.round(y),
                    w: Math.round(w),
                    h: Math.round(h),
    ...
    if (!window.__nativeBoundsWired) {
        window.addEventListener("resize", () => {
            try {
                window.__syncNativeBounds();
            } catch {}
        });
        window.__nativeBoundsWired = true;
    }
    window.__fitBrowserView = () => {
        try {
            const host = document.getElementById("webviewContainer");
    ...
    if (!window.__bvFitWired) {
        window.addEventListener("resize", () => {
            try {
                window.__fitBrowserView();
            } catch {}
        });
        window.__bvFitWired = true;
    }
    const w2gp = {
        platform: navigator.platform.includes("Win")
            ...
            : navigator.platform.includes("Mac")
              ? "darwin"
              : "linux",
        checkInstalled: () => call("check_installed"),
        detectGpu: () => call("detect_gpu"),
        detectGpus: () => call("detect_gpus"),
        detectHardware: () => call("detect_hardware"),
        getHardwareProfile: () => call("get_hardware_profile"),
        getSystemMetrics: () => call("get_system_metrics"),
        autoTuneDetect: () => call("auto_tune_detect"),
        autoTuneRecommend: (hw, opts) =>
            call("auto_tune_recommend", { hw, opts }),
        install: (envType) => call("install", { envType }),
        ...
        pythonPreflight: () => call("python_preflight"),
        restoreBackup: () => call("restore_backup"),
        manageList: () => call("manage_list"),
        manageSetActive: (name) => call("manage_set_active", { name }),
        uninstallEnv: (name) => call("uninstall_env", { name }),
        uvCacheInfo: () => call("uv_cache_info"),
        uvCacheSize: () => call("uv_cache_size"),
        uvCacheClean: (a) => call("uv_cache_clean", { action: a }),
        checkCommand: (cmd) => call("check_command", { cmd }),
        installPrerequisite: (tool) => call("install_prerequisite", { tool }),
        getInstallPaths: () => call("get_install_paths"),
        getDiskSpace: (path) => call("get_disk_space", { path }),
        openFolder: (p) => call("open_folder", { path: p }),
        setDataDir: (dir) => call("set_data_dir", { dir }),
        resetDataDir: () => call("reset_data_dir"),
        migrateToPreferred: (c) => call("migrate_to_preferred", { choices: c }),
        moveFolder: (src, dst) => call("move_folder", { src, dst }),
        migrateChoose: () => call("migrate_choose"),
        folderSize: (path) => call("folder_size", { path }),
        downloadsSince: (sinceMs) =>
            call("downloads_since", { sinceMs })
                .then((r) => (r && r.files) || [])
                .catch(() => []),
        saveDownloadedFile: (name, dir) =>
            call("save_downloaded_file", { name, dir: dir ?? null }),
        ...
        bvSyncBoundsRect: (r) =>
            call("bv_sync_bounds", r).catch(() => ({ ok: false })),
        // Browser-with-ask finish: staged file → native Save-As dialog → move.
        saveStagedDownload: (path, dir) =>
            call("save_staged_download", { path, dir: dir ?? null }),
        isNativeEmbed: () => nativeEmbed,
        onDownloadFinished: (cb) => {
            listen("download-finished", cb);
            return () => {};
        },
        onConsoleMirror: (cb) => {
        ...
        mirrorConsole: (text) => {
            try {
                call("mirror_console", { text }).catch(() => {});
            } catch {}
            return Promise.resolve({ ok: true });
        },
        onDownloadStarted: (cb) => {
            listen("download-started", cb);
            return () => {};
        },
        onGradioPageLoad: (cb) => {
            listen("gradio-page-load", cb);
            return () => {};
        },
        isDataDirRoaming: () => call("is_data_dir_roaming"),
        writeWgpConfig: (cfg) => call("write_wgp_config", { cfg }),
        selectFolder: () => call("select_folder"),
        resetWgpConfig: () => call("reset_wgp_config"),
        confirmDialog: async (opts) => {
            try {
                const r = await call("confirm_dialog", { opts });
                if (typeof r === "string") return r;
                if (r && typeof r.choice === "string") return r.choice;
                if (r && typeof r.response === "number")
                    return r.response === 0 ? "ok" : "cancel";
                if (r && r.ok) return "ok";
                return r;
            } catch {
                return "cancel";
        ...
        getModelPaths: () => call("get_model_paths"),
        ...
        createBrowserView: async (url, opts) => {
            ...
            try {
                ...
                const win = await WebviewWindow.getByLabel("wan2gp-view");
            ...
            let want = "native";
            try {
                const _cfg = await w2gp.configLoad().catch(() => ({}));
                want =
                    _cfg && _cfg.embedMode === "iframe" ? "iframe" : "native";
            } catch {}
            try {
                created = await call("create_browser_view", {
                    url: u,
                    opts: Object.assign({}, opts ?? null, { mode: want }),
                });
            } catch (e) {
                created = {
                    ok: false,
            ...
            if (want === "native" && (!created || created.mode !== "native"))
            ...
            if (nativeEmbed) {
                ...
                const host =
                    document.getElementById("webviewContainer") ||
                    document.body;
                if (host.id === "webviewContainer") {
                ...
                try {
                    const db = document.getElementById("dashBody");
            ...
            const host =
                document.getElementById("webviewContainer") || document.body;
            ...
            if (isWebviewHost) {
                ...
                host.style.flex = "1";
                host.style.minHeight = "0";
                host.style.height = "calc(100vh - 44px)";
                host.style.position = "relative";
            }
            const c = document.createElement("div");
            c.id = "tauri-browser-view";
            c.style.cssText =
                "flex:1;display:flex;flex-direction:column;background:#111;min-height:0;width:100%;height:100%;overflow:auto;";
            ...
            console.log(
                "[tauri] BrowserView iframe created for",
                u,
                "— if blank, check F12 Network for",
                u,
                "and X-Frame-Options",
            );
        ...
        hideBrowserView: async (reason) => {
            ...
            if (reason === "term") {
                console.log(
                    "[tauri] hideBrowserView(term) — keep iframe visible",
                );
                try {
                    await call("hide_browser_view");
                } catch {}
        ...
        destroyBrowserView: async () => {
            ...
            document.getElementById("tauri-browser-view")?.remove();
            try {
                const wc = document.getElementById("webviewContainer");
                if (wc) {
            ...
            try {
                await call("destroy_browser_view");
            } catch {}
            return { ok: true };
        },
        detachBrowserView: async () => {
            const c = document.getElementById("tauri-browser-view");
            if (c) c.style.display = "none";
            try {
                await call("detach_browser_view");
            } catch {}
            return { ok: true };
        },
        reattachBrowserView: async () => {
            const c = document.getElementById("tauri-browser-view");
            if (c) c.style.display = "flex";
            try {
                await call("reattach_browser_view");
            } catch {}
            return { ok: true };
        },
        createTermView: () => call("create_term_view"),
        destroyTermView: () => call("destroy_term_view"),
        ...
        onTermSetDock: (cb) => {
            listen("term-set-dock", cb);
            return () => {};
        },
        bvNavigate: (a) => call("bv_navigate", { action: a }),
        bvSetZoom: (f) => call("bv_set_zoom", { factor: f }),
        bvSetDock: (d) => {
            console.log("[tauri] bvSetDock", d, "— keep iframe visible");
            try {
                return call("bv_set_dock", { dock: d });
            } catch {
                return Promise.resolve({ ok: true });
            }
        },
        ...
        launchBrowser: async (url) => {
            const u = url || "http://localhost:7861";
            try {
                const r = await call("launch_browser", { url: u });
                if (r && (r.success || r.ok)) return r;
            } catch {}
            try {
                await window.__TAURI__.core.invoke("plugin:opener|open_url", {
                    url: u,
        ...
        launchBrowserNoGpu: async (url) => {
            const u = url || "http://localhost:7861";
            try {
                const r = await call("launch_browser_no_gpu", { url: u });
                if (r && (r.success || r.ok)) return r;
            } catch {}
            try {
                await window.__TAURI__.core.invoke("plugin:opener|open_url", {
        ...
        deepyWebStart: (mode, deepyPort, authMode, authFixed, https, publicUrl, extraArgs) =>
            call("deepy_web_start", {
                ...
                authMode,
                authFixed,
                httpsEnabled: !!(https && https.enabled),
        ...
        llmEngineServe: (e, a) =>
            call("llm_engine_serve", { engine: e, action: a }),
        llmEngineAuth: (e) => call("llm_engine_auth", { engine: e }),
        memoryProfileRead: () => call("memory_profile_read"),
        memoryProfileApply: (s) =>
            call("memory_profile_apply", { settings: s }),
        tsFailsafeApply: () => call("troubleshoot_failsafe_apply"),
        tsCudaCheck: () => call("troubleshoot_cuda_check"),
        tsPortStatus: () => call("troubleshoot_port_status"),
        tsPortFix: (a) => call("troubleshoot_port_fix", { action: a }),
        tsLongPathsStatus: () => call("troubleshoot_long_paths_status"),
        tsLongPathsEnable: () => call("troubleshoot_long_paths_enable"),
        tsDebugBundle: () => call("troubleshoot_debug_bundle"),
        tsTritonTest: () => call("troubleshoot_triton_test"),
        ...
        notifierEnsure: () => call("notifier_ensure"),
        notifierNativeStatus: () => call("notifier_native_status"),
        notifierNativeLoad: () => call("notifier_native_load"),
        notifierNativeSave: (c) => call("notifier_native_save", { cfg: c }),
        notifierNativeTest: (c) => call("notifier_native_test", { cfg: c }),
        dlss5Status: () => call("dlss5_status"),
        installDlss5: (force) => call("install_dlss5", { force: !!force }),
        pluginsList: () => call("plugins_list"),
        pluginInstall: (url) => call("plugin_install", { url }),
        pluginCheckUpdate: (id) => call("plugin_check_update", { id }),
        pluginCheckUpdates: () => call("plugin_check_updates"),
        ...
        setNotificationsEnabled: (e) =>
            call("set_notifications_enabled", { enabled: e }),
        checkUpdate: (o) => call("check_update", { opts: o }),
        downloadUpdate: (o) => call("download_update", { opts: o }),
        ...
        getCrashRecoveryInfo: () => call("get_crash_recovery_info"),
        uiModeSet: (m) => call("ui_mode_set", { mode: m }),
        // event bridges: Electron ipcRenderer.on → Tauri listen (returns unlisten fn) — with debug log for phases
        onSetupOutput: (cb) => {
            listen("setup-output", cb);
            return () => {};
        },
        onDlss5Progress: (cb) => {
            listen("dlss5-progress", cb);
            return () => {};
        },
        onInstallProgress: (cb) => {
            listen("install-progress", cb);
            return () => {};
        },
        onSetupPhase: (cb) => {
            console.log("[w2gp] onSetupPhase registered");
            listen("setup-phase", (p) => {
                console.log("[w2gp] setup-phase", p);
                try {
                    cb(p);
                } catch (e) {
                    console.error(e);
        ...
        onSetupProfile: (cb) => {
            listen("setup-profile", cb);
            return () => {};
        },
        onLaunchLog: (cb) => {
            listen("launch-log", cb);
            return () => {};
        },
        onWangpExit: (cb) => {
        ...
        onUpdateStatus: (cb) => {
            listen("update-status", cb);
            return () => {};
        },
        onBvNavState: (cb) => {
            listen("bv-nav-state", cb);
            return () => {};
        },
        onTermDockChanged: (cb) => {
            listen("term-dock-changed", cb);
            return () => {};
        },
        onTermClosed: (cb) => {
            listen("term-closed", cb);
            return () => {};
        },
        onOpenMigration: (cb) => {
            listen("open-migration", cb);
            return () => {};
        },
        onMigrationProgress: (cb) => {
            listen("migration-progress", cb);
            return () => {};
        },
