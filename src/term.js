...
// Theme + appearance: the main window stores dark/light + accent/scales in
// desktop-config.json and mirrors them to localStorage — this window's
...
(() => {
  try {
    const a = localStorage.getItem("w2gp.accent");
    if (a && a !== "mono")
      document.documentElement.setAttribute("data-accent", a);
    const ui = parseInt(localStorage.getItem("w2gp.uiScale") || "", 10);
    if (ui >= 85 && ui <= 130)
      document.documentElement.style.setProperty("--ui-scale", ui / 100);
    const tm = parseInt(localStorage.getItem("w2gp.termScale") || "", 10);
    if (tm >= 85 && tm <= 150)
      document.documentElement.style.setProperty("--term-scale", tm / 100);
  } catch {}
})();
...
document.querySelectorAll(".dock-btn").forEach((b) => {
  b.addEventListener("click", async () => {
    ...
    try {
      appendToBuf("[term] dock → " + d);
    } catch {}
    termStatus("switching to " + d + "…");
    try {
      await window.w2gp.setDock(d);
      termStatus("sent ✓ — waiting for main…");
    } catch (e) {
      const m = (e && e.message) || e;
      termStatus("FAILED: " + m);
      try {
        appendToBuf("[term] dock FAILED: " + m);
      } catch {}
    }
  });
});
document
  .getElementById("ftCloseBtn")
  .addEventListener("click", () => window.w2gp.closeTerm());
document
  .getElementById("logExportBtn")
  .addEventListener("click", () =>
    window.w2gp.exportLogs(
      buf.join("\n") + (_lastLine ? "\n" + _lastLine : ""),
    ),
  );
